...
#define VENDOR_BUILD_ID_STR "ro.vendor.build.id="
#define VENDOR_BUILD_ID_STR_LEN 19
#define VENDOR_BUILD_ID_BUF_LEN 16
...
/* verify_vendor("VENDOR_BUILD_ID") */
Value* VerifyVendorFn(const char* name, State* state,
                      const std::vector<std::unique_ptr<Expr>>& argv) {
    char current_vendor_build_id[VENDOR_BUILD_ID_BUF_LEN];
    int ret;

    // Check for build id
    ret = get_info(current_vendor_build_id, VENDOR_BUILD_ID_BUF_LEN, VENDOR_BUILD_ID_STR,
                   VENDOR_BUILD_ID_STR_LEN, VENDOR_PART_PATH);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
                          "%s() failed to read current vendor build id: %d", name, ret);
    }
    ...
    if (std::equal(args[0].begin(), args[0].end(), std::string(current_vendor_build_id).begin())) {
        ret = 1;
    }
...
void Register_librecovery_updater_huawei() {
    RegisterFunction("huawei.verify_vendor_build_id", VerifyVendorFn);
}
